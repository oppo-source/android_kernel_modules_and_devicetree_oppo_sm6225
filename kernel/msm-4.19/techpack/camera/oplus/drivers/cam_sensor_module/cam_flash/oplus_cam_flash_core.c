...
int cam_flash_on(struct cam_flash_ctrl *flash_ctrl,
	...
	int mode) {
	int rc = 0;
	if ((NULL != flash_ctrl->soc_info.flash_name) && (!strncmp(flash_ctrl->soc_info.flash_name, "sgm3785", sizeof("sgm3785"))))
	{
		if (mode == 0) {
			if (flash_ctrl->flash_type == 2) {
				cam_flash_gpio_power_ops_sgm3785(flash_ctrl,true);
				rc = cam_flash_gpio_low_sgm3785(flash_ctrl, flash_data);
			} else {
			...
			if (flash_ctrl->flash_type == 2) {
				cam_flash_gpio_power_ops_sgm3785(flash_ctrl,true);
				rc = cam_flash_gpio_high_sgm3785(flash_ctrl, flash_data);
			} else {
