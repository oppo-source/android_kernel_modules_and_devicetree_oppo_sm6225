		...
		if (fingerprint_mode) {
			...
			if(OPLUS_DISPLAY_AOD_HBM_SCENE == get_oplus_display_scene()) {
				if (OPLUS_DISPLAY_POWER_DOZE_SUSPEND == get_oplus_display_power_status() ||
					...
					if ((!strcmp(dsi_display->panel->name, "samsung s6e3fc3 amoled fhd cmd panel")) ||
					    (!strcmp(dsi_display->panel->name, "s6e3fc3 samsung amoled fhd+ cmd mode dsi samsung panel"))) {
