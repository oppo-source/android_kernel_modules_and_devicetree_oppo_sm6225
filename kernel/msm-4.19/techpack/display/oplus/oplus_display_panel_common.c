...
int oplus_panel_update_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if ((!strcmp(panel->name, "samsung s6e3fc3 amoled fhd cmd panel")) ||
		(!strcmp(panel->name, "s6e3fc3 samsung amoled fhd+ cmd mode dsi samsung panel"))) {
		if ((get_oplus_display_scene() == OPLUS_DISPLAY_AOD_SCENE) && (oplus_dimlayer_hbm != 0)) {
			pr_info("dsi_cmd OPLUS_DISPLAY_AOD_SCENE return bl_lvl:%d\n", bl_lvl);
			goto skip_bl_update;
		}
		if ((get_oplus_display_scene() == OPLUS_DISPLAY_AOD_HBM_SCENE) && (oplus_dimlayer_hbm != 0)) {
			pr_info("dsi_cmd OPLUS_DISPLAY_AOD_HBM_SCENE return bl_lvl:%d\n", bl_lvl);
			goto skip_bl_update;
		}
	}
	...
	if (panel->oplus_priv.is_aod_ramless)
		...
		if (panel->oplus_priv.is_samsung_panel && (get_oplus_display_scene() != OPLUS_DISPLAY_AOD_SCENE)) {
