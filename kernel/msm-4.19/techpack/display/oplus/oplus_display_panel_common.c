...
int oplus_panel_update_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	/* do not set backlight if there is aod layer, to avoid aod flicker */
	if(oplus_aod_layer_status != -1) {
		pr_info("aod layer status = %d, backlight = %d, skip backlight update\n", oplus_aod_layer_status, bl_lvl);
		goto skip_bl_update;
	}
