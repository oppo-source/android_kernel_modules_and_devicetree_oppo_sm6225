...
extern int oplus_aod_layer_status;

int oplus_last_aod_layer_status;
...
int oplus_aod_off_backlight_recovery(void *drm_connector) {
	int rc = 0;
	struct drm_connector *drm_conn = drm_connector;
	struct sde_connector *c_conn = NULL;
	struct dsi_display *display = NULL;

	if(!drm_conn) {
		SDE_ERROR("Invalid drm_connector params\n");
		return -EINVAL;
	}

	c_conn = to_sde_connector(drm_conn);
	if (!c_conn) {
		SDE_ERROR("Invalid c_conn params\n");
		return -EINVAL;
	}

	if (c_conn->connector_type != DRM_MODE_CONNECTOR_DSI) {
		SDE_ERROR("not in dsi mode, should not recovery backlight after aod off\n");
		return 0;
	}
	display = c_conn->display;

	if (!display || !display->panel) {
		SDE_ERROR("Invalid display params\n");
		return -EINVAL;
	}

	if (oplus_last_aod_layer_status != -1 && oplus_aod_layer_status == -1) {
		SDE_ERROR("recovery backlight level after aod off\n");
		rc = dsi_panel_set_backlight(display->panel, display->panel->bl_config.bl_level);
		if (rc) {
			SDE_ERROR("unable to set backlight\n");
		}
	}

	oplus_last_aod_layer_status = oplus_aod_layer_status;
