		...
		if ((display->panel->power_mode == SDE_MODE_DPMS_LP1) ||
			...
			if (sde_crtc_get_fingerprint_mode(connector->state->crtc->state)) {
				...
				if (display->panel->panel_initialized && !display->panel->is_hbm_enabled) {
					if((!strcmp(display->panel->name, "samsung s6e3fc3 amoled fhd cmd panel")) ||
						(!strcmp(display->panel->name, "s6e3fc3 samsung amoled fhd+ cmd mode dsi samsung panel"))) {
				...
				if((!strcmp(display->panel->name, "samsung s6e3fc3 amoled fhd cmd panel")) ||
					(!strcmp(display->panel->name, "s6e3fc3 samsung amoled fhd+ cmd mode dsi samsung panel")))	 {
					pr_info("display->panel->name:%s resend bl_level :%d\n", display->panel->name, display->panel->bl_config.bl_level);
					mutex_lock(&display->panel->panel_lock);
