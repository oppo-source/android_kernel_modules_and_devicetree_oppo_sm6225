...
static int lcd_bl_write_byte(unsigned char addr, unsigned char value)
{
	...
	if (NULL == g_i2c_client) {
		LCD_BL_PRINT("[LCD][BL] g_i2c_client is null!!\n");
		return -EINVAL;
	}
...
static int lcd_bl_read_byte(u8 regnum)
{
	...
	buffer[0] = regnum;
	res = i2c_master_send(g_i2c_client, buffer, 0x1);
	if (res <= 0)	{
		mutex_unlock(&read_lock);
		LCD_BL_PRINT("read reg send res = %d\n", res);
		return res;
	}
	res = i2c_master_recv(g_i2c_client, reg_value, 0x1);
	if (res <= 0) {
		mutex_unlock(&read_lock);
...
{
	printk("lcd_bl_enable,value = %d", enable);
	if (enable) {
		lcd_bl_write_byte(0x02, 0x92);/* BL_CFG1；OVP=34V，指数调光，PWM enable */
		lcd_bl_write_byte(0x03, 0xCD);/* diming 256ms PWM_HYST 10 LSBs*/
		lcd_bl_write_byte(0x14, 0x14);
		lcd_bl_write_byte(0x11, 0xB7);/* BL_OPTION2；电感4.7uH，BL_CURRENT_LIMIT 2.5A；*/
		lcd_bl_write_byte(0x15, 0xA9);/* Backlight Full-scale LED Current 24.4mA/CH；*/
		lcd_bl_write_byte(0x08, 0x7F);/* BL enabled and Current sink 1/2/3/4 /5 enabled；*/
	} else {
		lcd_bl_write_byte(0x08, 0x00);/* Disable BL */
	}
	return 0;
}
extern unsigned int lcd_bl_first_enable_flag;
int lcd_bl_set_led_brightness(int value)//for set bringhtness
{
	...
	if (value > 0) {
		...
		lcd_bl_first_enable_flag = 0;
		lcd_bl_enable(true);} //bl enabled /* BL enabled and Current sink 1/2/3/4 enabled；*/
	...
	if (value == 0) {
		...
		lcd_bl_enable(false); //bl disenabled /* BL enabled and Current sink 1/2/3/4 enabled；*/
		lcd_bl_first_enable_flag = 1;
	}
...
{
	printk("--lcd, lcd_disable_bias function,value = %d", enable);
	if (enable) {
		lcd_bl_write_byte(0x0C, 0x2C);/* LCD_BOOST_CFG */
		lcd_bl_write_byte(0x0D, 0x24);/* OUTP_CFG，OUTP = 5.5V */
		lcd_bl_write_byte(0x0E, 0x24);/* OUTN_CFG，OUTN = -5.5V */
		lcd_bl_write_byte(0x09, 0x9E);/* enable OUTP and OUTN */
		mdelay(11);
	} else {
...
static int ktz8866_pwr_parse_gpios(struct device *dev)
{
	...
	gpio_arrays[KTZ8866_HW_EN].gpio_name = "panel,ktz8866-hwen-gpio";
	gpio_arrays[PANEL_VIO_EN].gpio_name = "panel,panel-vddi-gpio";
...
int turn_on_ktz8866_hw_en(bool on)
{
	int ret;
	u8 value  = on ? 1 : 0;
	printk("--lcd, turn_on_ktz8866_hw_en function,value = %d", value);
...
int turn_on_panel_vio_en(bool on)
{
	int ret;
	u8 value  = on ? 1 : 0;
	printk("--lcd, turn_on_panel_vio_en function,value = %d", value);

	if(gpio_is_valid(gpio_arrays[PANEL_VIO_EN].gpio_num)) {
		ret = gpio_direction_output(gpio_arrays[PANEL_VIO_EN].gpio_num, value);
		if(ret){
			pr_err("failed to set %s gpio %d, ret = %d\n", gpio_arrays[PANEL_VIO_EN].gpio_name, value, ret);
			return ret;
		}
	} else {
		pr_err("PANEL_VIO_EN is not vaild,PANEL_VIO_EN is %d\n", gpio_arrays[PANEL_VIO_EN].gpio_num);
	}

	return 0;
}
EXPORT_SYMBOL(turn_on_panel_vio_en);

static int lcd_power_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret;
	printk("lcd_power_probe +++++!\n");
...
#if 0
	ret = turn_on_panel_vio_en(true);
	if(ret) {
		pr_err("failed to turn on panel_vio_en!\n");
	}
...
{
	printk("lcd_ktz8866_init\n");

	if (i2c_add_driver(&lcd_ktz8866_i2c_driver)) {
		printk("[LCD][BL] Failed to register lcd_ktz8866_i2c_driver!\n");
		return -EINVAL;
	}
