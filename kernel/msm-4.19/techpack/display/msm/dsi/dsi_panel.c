...
extern int __attribute__((weak)) tp_control_cs_gpio(bool enable) {return 0;};
...
bool is_lcd_panel(const char *panel_name)
{
	if (!strcmp(panel_name,"ili9883a 90hz boe fhd video mode dsi panel") ||
		...
		!strcmp(panel_name,"ili9883c 90hz boe hd video mode dsi panel") ||
		!strcmp(panel_name,"ili9883c 90hz hlt hd video mode dsi panel"))
		return true;
	return false;
}

static bool need_disable_tp_irq(const char *panel_name) {
	if (!strcmp(panel_name,"ili9883a 90hz boe fhd video mode dsi panel") ||
		!strcmp(panel_name,"ili7807s boe fhd video mode dsi panel") ||
		!strcmp(panel_name,"ili7807s hlt fhd video mode dsi panel") ||
		!strcmp(panel_name,"ili9883c 90hz boe hd video mode dsi panel") ||
...
static bool need_reset_lcd_tp(const char *panel_name) {
	if (strcmp(panel_name,"ili9883c 90hz boe hd video mode dsi panel") &&
			strcmp(panel_name,"ili9883c 90hz hlt hd video mode dsi panel"))
		return true;
	return false;
}
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if ((mode != MSM_BOOT_MODE__FACTORY)
		...
		pr_info("[TP] now disp power normal on, need to enable gpio and load fw\n");
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
...
#ifdef OPLUS_BUG_STABILITY
	pr_info("dsi_panel_power_off\n");
	if (panel->oplus_panel_power.oplus_panel_power_ctl) {
		g_tp_gesture_enable_flag = tp_gesture_enable_flag();
		pr_info ("[%s] oplus_panel_power_ctl off g_tp_gesture_enable_flag = %s\n",
			__func__, g_tp_gesture_enable_flag ? "true" : "false");
		rc = oplus_panel_off(panel);
		if (rc)
			pr_err("[%s]oplus_panel_power_ctl off fail, rc=%d\n", __func__, rc);
		return rc;
	}

	if ((is_lcd_panel(panel->name) && !tp_gesture_enable_flag()) ||
			atomic_read(&(panel->esd_recovery_pending)) || (g_shutdown_flag == 1)) {
		if (strcmp(panel->name,"ili9883a 90hz boe fhd video mode dsi panel") || (g_shutdown_flag == 1)) {
			if (need_reset_lcd_tp(panel->name) || (g_shutdown_flag == 1)) {
				if (gpio_is_valid(panel->reset_config.reset_gpio))
					gpio_set_value(panel->reset_config.reset_gpio, 0);
			}

			if (need_disable_tp_irq(panel->name) && (g_shutdown_flag == 1)) {
				tp_irq = gpio_to_irq(1279);
				disable_irq_nosync(tp_irq);
			}
			usleep_range(100*1000, (100*1000) + 100);

			/* add for not to pull down reset and tp reset while Non-shutdown power off */
			if (need_reset_lcd_tp(panel->name) || (g_shutdown_flag == 1)) {
				tp_gpio_current_leakage_handler(false);
				tp_control_reset_gpio(0);
				tp_control_cs_gpio(0);
			}

			if (strcmp(panel->name,"ili9883a 90hz boe fhd video mode dsi panel") && atomic_read(&(panel->esd_recovery_pending))) {
	...
	/*add for fast off/on extern regulator need delay issue*/
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-pixel-off-command-state",
#endif /*OPLUS_BUG_STABILITY*/
};
...
#ifdef OPLUS_BUG_STABILITY
/* add 1ms delay if lp11 not enable */
	else {
		if (panel->oplus_panel_power.oplus_panel_power_ctl)
