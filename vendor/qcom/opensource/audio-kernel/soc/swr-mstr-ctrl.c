				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (//Check the register address, modifications for fixing crash issue
            !(param[0] % 4) &&
             (param[0] <= SWR_MSTR_MAX_REG_ADDR) && (rc == 0))
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (//Check the register address, modification for fixing crash issue
	     !(param[0] % 4) &&
             (param[0] <= SWR_MSTR_MAX_REG_ADDR) &&
		(param[1] <= 0xFFFFFFFF) &&
		(rc == 0))
