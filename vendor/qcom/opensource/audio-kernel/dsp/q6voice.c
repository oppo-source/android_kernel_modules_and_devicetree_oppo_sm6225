...
static int voice_destroy_vocproc(struct voice_data *v)
{
	...
	if (v->voc_state == VOC_RUN) {
		oplus_voice_get_opve_err_fb(apr_cvp, v);
	}
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		if ((OPVE_RX_MODULE_ID == ptr[1]) && (INSTANCE_ID_0 == ptr[2]) && (VOICE_PARAM_OPVE_GET_ERR == ptr[3])) {
			 /* ptr[4] is return payload bytes, ptr[5] is the start of payload data */
			if ((ptr[4] > 0) && (data->payload_size > (5 * sizeof(uint32_t)))) {
				oplus_opve_copy_voice_err_result(MSM_AFE_PORT_TYPE_RX, (void *)(ptr + 5), data->payload_size - (5 * sizeof(uint32_t)));
			}
		} else if ((OPVE_TX_MODULE_ID == ptr[1]) && (0x8000 == ptr[2]) && (VOICE_PARAM_OPVE_GET_ERR == ptr[3])) {
			if ((ptr[4] > 0) && (data->payload_size > (5 * sizeof(uint32_t)))) {
				oplus_opve_copy_voice_err_result(MSM_AFE_PORT_TYPE_TX, (void *)(ptr + 5), data->payload_size - (5 * sizeof(uint32_t)));
			}
		}
#endif
