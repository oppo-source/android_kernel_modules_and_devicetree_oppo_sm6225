...
int adm_close(int port_id, int perf_mode, int copp_idx)
{
	...
	if ((atomic_read(&this_adm.copp.app_type[port_idx][copp_idx]) == VOICE_OR_VOIP_APP_TYPE) && \
		((atomic_read(&this_adm.copp.topology[port_idx][copp_idx]) == VOICE_TOPOLOGY_OPVE_TX_SM) || \
			(atomic_read(&this_adm.copp.topology[port_idx][copp_idx]) == VOICE_TOPOLOGY_OPVE_TX_DM) || \
			(atomic_read(&this_adm.copp.topology[port_idx][copp_idx]) == VOICE_TOPOLOGY_OPVE_TX_QM) || \
			(atomic_read(&this_adm.copp.topology[port_idx][copp_idx]) == VOICE_TOPOLOGY_OPVE_RX) || \
			(atomic_read(&this_adm.copp.topology[port_idx][copp_idx]) == AUDIO_TOPOLOGY_OPVE_RX))
		) {
		oplus_adm_get_opve_err_fb(port_id, copp_idx);
	}
#endif
