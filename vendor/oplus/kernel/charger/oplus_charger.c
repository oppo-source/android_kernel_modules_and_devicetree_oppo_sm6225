...
#define SALE_MODE_COOL_DOWN_ONE 501
...
extern int oplus_thermal_tmp_get_batt(void);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			...
			&& oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC) {
				...
				if (oplus_chg_get_voocphy_support() == NO_VOOCPHY || !chip->charger_exist) {
...
void oplus_chg_smooth_switch_v2_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chg_info("smooth_switch_v2 %d reserve_soc %d \n", chip->rsd.smooth_switch_v2, chip->rsd.reserve_soc);
}
...
void oplus_chg_usbtemp_v2_parse_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,usbtemp_temp_gap_low_with_batt_temp", &chip->usbtemp_temp_gap_low_with_batt_temp);
	if (rc) {
		chip->usbtemp_temp_gap_low_with_batt_temp = OPCHG_USBTEMP_GAP_LOW_WITH_BATT_TEMP;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_temp_gap_high_with_batt_temp", &chip->usbtemp_temp_gap_high_with_batt_temp);
	if (rc) {
		chip->usbtemp_temp_gap_high_with_batt_temp = OPCHG_USBTEMP_GAP_HIGH_WITH_BATT_TEMP;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_temp_gap_low_without_batt_temp", &chip->usbtemp_temp_gap_low_without_batt_temp);
	if (rc) {
		chip->usbtemp_temp_gap_low_without_batt_temp = OPCHG_USBTEMP_GAP_LOW_WITHOUT_BATT_TEMP;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_temp_gap_high_without_batt_temp", &chip->usbtemp_temp_gap_high_without_batt_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,usbtemp_rise_fast_temp_count_low", &chip->usbtemp_rise_fast_temp_count_low);
	if (rc) {
		chip->usbtemp_rise_fast_temp_count_low = OPCHG_USBTEMP_RISE_FAST_TEMP_COUNT_LOW;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_rise_fast_temp_count_high", &chip->usbtemp_rise_fast_temp_count_high);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recover_ntc_low", &chip->usbtemp_cool_down_recover_ntc_low);
	if (rc) {
		chip->usbtemp_cool_down_recover_ntc_low = OPCHG_USBTEMP_COOL_DOWN_RECOVER_NTC_LOW;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recover_ntc_high", &chip->usbtemp_cool_down_recover_ntc_high);
	if (rc) {
		chip->usbtemp_cool_down_recover_ntc_high = OPCHG_USBTEMP_COOL_DOWN_RECOVER_NTC_HIGH;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recover_gap_low", &chip->usbtemp_cool_down_recover_gap_low);
	if (rc) {
		chip->usbtemp_cool_down_recover_gap_low = OPCHG_USBTEMP_COOL_DOWN_RECOVER_GAP_LOW;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recover_gap_high", &chip->usbtemp_cool_down_recover_gap_high);
	if (rc) {
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->voocphy_support_display_vooc = of_property_read_bool(node, "qcom,voocphy_support_display_vooc");
	if (get_eng_version() == HIGH_TEMP_AGING
	...
	chip->tbatt_use_subboard_temp = of_property_read_bool(node, "oplus,tbatt_use_subboard_temp");
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((oplus_voocphy_get_btb_temp_over() || oplus_vooc_get_btb_temp_over()) &&
		...
		chg_err(" oplus_voocphy_get_btb_temp_over ,set current_limit 1A\n");
	}

	if (chg_ctrl_by_sale_mode) {
		if (chip->cool_down == SALE_MODE_COOL_DOWN_ONE)
...
/*	chip->pmic_spmi.usb_online = false;
	...
	if(chip->external_gauge) {
		chg_err("use oplus_gauge_get_batt_authenticate\n");
		chip->authenticate = oplus_gauge_get_batt_authenticate();
		chip->hmac = oplus_gauge_get_batt_hmac();
	} else {
		chg_err("use get_oplus_high_battery_status\n");
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
		...
		if (chip->ui_soc == 100 && (chip->rsd.smooth_switch_v2 || (chip->fastchg_to_ffc == false)) &&
		    (oplus_vooc_get_fastchg_started() == false ||
		     oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC)) {
			if (++cnt >= 12) {
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_vooc_get_fastchg_started() == false
			...
			if ((oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY || oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY)
				...
				oplus_chg_unsuspend_charger();
				msleep(500);
				chg_debug("%s, reset\n", high_vol_light == true ?"high_vol_light ":"dummy");
		...
		if (!oplus_vooc_get_fastchg_started()
			...
			if (reset_mcu_delay == RESET_MCU_DELAY_15S) {
				...
				if (chip->charger_volt <= 7500
						&& (!chip->chg_ops->oplus_chg_get_pd_type ||chip->chg_ops->oplus_chg_get_pd_type() != PD_PPS_ACTIVE)) {
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (chip->smart_charge_version == 1) {
		...
		if (!val) {
			...
			return;
		} else if ((val == SALE_MODE_COOL_DOWN_ONE || val == SALE_MODE_COOL_DOWN_TWO)
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		if (g_charger_chip->support_subboard_ntc) {
			g_charger_chip->subboard_temp = oplus_thermal_tmp_get_batt() / 100;
			batt_temp = oplus_gauge_get_batt_temperature();
			if (get_eng_version() == HIGH_TEMP_AGING) {
				chg_err("[OPLUS_CHG]CONFIG_HIGH_TEMP_VERSION " "enable here, disable high tbat shutdown\n");
				if (batt_temp > 690)
					batt_temp = 690;
