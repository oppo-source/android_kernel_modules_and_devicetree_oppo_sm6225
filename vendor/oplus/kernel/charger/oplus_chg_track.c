...
#define TRACK_ALLOW_READING_ERR			100
...
static struct oplus_chg_track_chg_abnormal_reason chg_abnormal_reason_table[] = {
	...
	{ TRACK_ALLOW_READING_ERR, "allow_reading_err", 0 },
};
...
static struct oplus_chg_track_fastchg_break ap_voocphy_break_table[] = {
	...
	{ TRACK_CP_VOOCPHY_LOW_CURR_ERR, "low_curr_err" },
	...
	struct oplus_chg_track_status *track_status)
{
	...
	if(track_status->mmi_chg_open_t) {
		...
		index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				";open_t,%d;", track_status->mmi_chg_open_t);
		index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				"close_t,%d", track_status->mmi_chg_close_t);
		index += snprintf(&(p_trigger_data->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
				"constant_t,%d", track_status->mmi_chg_constant_t);
	}
...
static int oplus_chg_track_init(struct oplus_chg_track *track_dev)
{
	...
	chip->wls_charging_break_trigger.type_reason =
		TRACK_NOTIFY_TYPE_CHARGING_BREAK;
	chip->plugout_state_trigger.type_reason =
	...
	chip->track_status.debug_allow_reading = 0;
	...
	struct oplus_chg_track_status *track_status)
{
	...
	if (oplus_vooc_get_fastchg_started() == false &&
		...
		if (track_status->allow_reading_err > 5)
			oplus_chg_track_get_chg_abnormal_reason_info(TRACK_ALLOW_READING_ERR, track_status);
	} else {
		if (track_status->debug_allow_reading) {
			oplus_chg_track_get_chg_abnormal_reason_info(TRACK_ALLOW_READING_ERR, track_status);
			track_status->debug_allow_reading = 0;
		}
...
static int oplus_chg_track_debugfs_init(struct oplus_chg_track *track_dev)
{
	...
	debugfs_create_u8("debug_allow_reading", 0644, debugfs_general,
			   &(track_dev->track_status.debug_allow_reading));
