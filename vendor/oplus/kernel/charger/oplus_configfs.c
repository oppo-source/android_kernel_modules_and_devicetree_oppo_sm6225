...
static ssize_t fast_chg_type_store(struct device *dev, struct device_attribute *attr, const char *buf,
                size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
                ...
                char *buf)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_PD_SDP || (CHARGER_SUBTYPE_PD == type &&
...
static ssize_t charge_technology_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
