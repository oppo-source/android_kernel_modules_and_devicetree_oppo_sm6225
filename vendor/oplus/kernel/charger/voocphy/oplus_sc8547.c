...
#define I2C_DEFAULT_LENGTH    8
#define I2C_READ_DELAY_US     350000
...
static void sc8547_update_data(struct oplus_voocphy_manager *chip)
{
	...
	/* parse data_block1 or data_block2 for improving time of interrupt */
	i2c_smbus_read_i2c_block_data(chip->client, SC8547_REG_19, 4, data_block1);
	i2c_smbus_read_i2c_block_data(chip->client, SC8547_REG_13, 4, data_block2);

	chip->int_flag = data;
	chip->cp_vsys = ((data_block1[0] << 8) | data_block1[1]) * 125 / 100;
	chip->cp_vbat = ((data_block1[2] << 8) | data_block1[3]) * 125 / 100;
	chip->cp_ichg = ((data_block2[0] << 8) | data_block2[1]) * 1875 / 1000;
	chip->cp_vbus = ((data_block2[2] << 8) | data_block2[3]) * 375 / 100;
...
int sc8547_get_cp_vbus(struct oplus_voocphy_manager *chip)
{
	...
	ret = sc8547_read_byte(chip->client, SC8547_REG_0E, &data);
	if (ret < 0) {
		pr_err("read SC8547_REG_0E error\n");
		return false;
	}

	ret = sc8547_get_adc_enable(chip, &adc_enable);
	if (ret < 0) {
		pr_err("read SC8547_REG_07 error\n");
		return false;
	}

	if (data & SC8547_ADAPTER_INSERT_STAT_MASK) {
		/*int_flag*/
		if (!adc_enable) {
			sc8547_write_byte(chip->client, SC8547_REG_11, SC8547_VBUS_ADC_CTRL_ENABLE);
			usleep_range(I2C_READ_DELAY_US, I2C_READ_DELAY_US);
		}
		...
		/*parse data_block for improving time of interrupt*/
		ret = i2c_smbus_read_i2c_block_data(chip->client, SC8547_REG_13, I2C_DEFAULT_LENGTH, data_block);
		if (ret < 0) {
			pr_err("read SC8547_REG_13 block data error \n");
		}
		cp_vbus = ((data_block[2] << SC8547_VBUS_ADC_SW_DIS_SHIFT) | data_block[3]) * SC8547_VBUS_ADC_SW_LSB_H / SC8547_VBUS_ADC_SW_LSB_L;
		pr_info("vbus present, cp_vbus = %d\n", cp_vbus);
	}else {
		pr_info("vbus not present, cp_vbus = %d\n", cp_vbus);
	}
	return cp_vbus;
}
