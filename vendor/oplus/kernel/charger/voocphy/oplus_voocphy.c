...
#define FAILED_FASTCHG_FLAG		0xF
#define NOT_IN_FASTCHG_FLAG		0x0
...
int oplus_voocphy_get_cp_vbus(void)
{
	...
	if (g_voocphy_chip->vooc_flag != NOT_IN_FASTCHG_FLAG && g_voocphy_chip->vooc_flag != FAILED_FASTCHG_FLAG
			&& g_voocphy_chip->fastchg_start) {
		cp_vbus = g_voocphy_chip->cp_vbus;
	} else {
...
void oplus_voocphy_handle_voocphy_status(struct work_struct *work)
{
	...
	if (intval == FAST_NOTIFY_PRESENT) {
		...
		voocphy_info("fastchg_start: [%d] dummy_start [%d] fastchg_stage [%d] \n",
			chip->fastchg_start, chip->fastchg_dummy_start, chip->fastchg_stage);
