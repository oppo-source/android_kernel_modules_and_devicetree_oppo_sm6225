...
static void bq28z610_bcc_set_buffer(int *buffer)
{
	...
	if (true == oplus_is_voocphy_charging()) {
...
static int bq28z610_get_battery_bcc_parameters(char *buf)
{
	...
	if ((oplus_vooc_get_fastchg_ing() && oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC &&
	    !oplus_vooc_get_fastchg_dummy_started()) || (true == oplus_voocphy_get_fastchg_ing())) {
		buffer[15] = 1;
	} else {
		buffer[15] = 0;
	}
	...
	chg_err(KERN_ERR
		...
		oplus_vooc_bcc_get_temp_range(), oplus_voocphy_get_fastchg_ing(), oplus_voocphy_bcc_get_temp_range());
	...
	if (bcc_debug_mode & BCC_Y_DEBUG) {
		...
		chg_err(KERN_ERR "%s bcc_debug_buf:%s\n", __func__, bcc_debug_buf);
		return 0;
	}
#endif

	chg_err(KERN_ERR "%s buf:%s\n", __func__, buf);
...
/*modify for update in real time*/
	...
	chg_err(KERN_ERR "%s bcc_voocphy buf:%d,%d,%d,%d,%d,%d\n", __func__, oplus_vooc_get_fastchg_ing(),
