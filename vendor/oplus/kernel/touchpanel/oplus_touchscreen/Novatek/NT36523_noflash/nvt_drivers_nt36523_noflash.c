...
/*Longfei.Wang@ODM_WT.BSP.TP,2021/12/24,add for WT hardwareinfo*/
...
//static int32_t nvt_ts_pen_data_checksum(uint8_t *buf, uint8_t length);   //by wanglongfei 20211211
...
info_retry:
	...
	/*Longfei.Wang@ODM_WT.BSP.TP,2021/12/24,add for WT hardwareinfo*/
	sprintf(Ctp_name, "INX,NT36525,FW:0x%02x", buf[1]);
	TPD_INFO("nvt_get_fw_info_noflash Ctp_name is : %s\n", Ctp_name);
...
static void nvt_get_pen_points(void *chip_data, struct pen_info *points)
{
	...
	if (pen_format_id != 0xFF) {
		if (pen_format_id == 0x01) {
			...
			points->tilt_x = (int32_t)point_data[73];
			points->tilt_y = (int32_t)point_data[74];
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	//case PEN_DETECT:    //by wanglongfei 20211211
...
static int nvt_mode_switch(void *chip_data, work_mode mode, bool flag)
{
    ...
    /*case MODE_PEN_SCAN:    //by wanglongfei 20211211
...
//    .get_pen_points             = nvt_get_pen_points,    //by wanglongfei 20211211
...
static int nvt_tp_probe(struct spi_device *client)
{
	...
	client->controller_data = (void *)&qcom_ctrl_data;   /*SPI CS hold time less 200ns*/
