...
void syna_tcm_refresh_switch(int fps, int tp_index)
{
	int retval = 0;

	if (!g_tcm_info[tp_index]) {
		return;
	}

	g_tcm_info[tp_index]->display_refresh_rate = fps;

	if (!*g_tcm_info[tp_index]->in_suspend && !g_tcm_info[tp_index]->game_mode) {
		retval = syna_tcm_set_dynamic_config(g_tcm_info[tp_index], DC_SET_REPORT_FRE,
						     g_tcm_info[tp_index]->display_refresh_rate == 60 ? 0x01 : 0x02);
...
static int syna_tcm_set_game_mode(struct syna_tcm_data *tcm_info, int enable)
{
    ...
    if (enable) {
		...
		retval = syna_tcm_set_dynamic_config(tcm_info, DC_SET_REPORT_FRE,
						     tcm_info->display_refresh_rate == 60 ? 0x01 : 0x02);
...
static int syna_tcm_normal_mode(struct syna_tcm_data *tcm_info)
{
	...
	syna_tcm_refresh_switch(tcm_info->display_refresh_rate, tcm_info->tp_index);
...
static void syna_main_register(struct seq_file *s, void *chip_data)
{
	...
	retval = syna_tcm_get_dynamic_config(tcm_info,
					     DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, &config);
