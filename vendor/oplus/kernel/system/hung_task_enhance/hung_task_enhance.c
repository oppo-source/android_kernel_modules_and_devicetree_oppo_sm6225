...
/* DeathHealer, kill D/T/t state tasks */
	...
	if (is_usersapce_key_process(t))
	{
		if (t->state == TASK_UNINTERRUPTIBLE)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_OPPO_KILL_LEN, "%s,uninterruptible for %ld seconds", t->comm, timeout);
		else if (t->state == TASK_STOPPED)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_OPPO_KILL_LEN, "%s,stopped for %ld seconds", t->comm, timeout);
		else if (t->state == TASK_TRACED)
			snprintf(sysctl_hung_task_kill, HUNG_TASK_OPPO_KILL_LEN, "%s,traced for %ld seconds", t->comm, timeout);
		else
			snprintf(sysctl_hung_task_kill, HUNG_TASK_OPPO_KILL_LEN, "%s,unknown hung for %ld seconds", t->comm, timeout);
