...
/* CONFIG_MMC_SDHCI_BH201 change the array to global */
...
/* CONFIG_MMC_SDHCI_BH201 change the function to global*/
...
/* CONFIG_MMC_SDHCI_BH201 change the function global*/
...
{
	/* Add for timeout controller register setting debug */
...
/* #ifdef CONFIG_MMC_SDHCI_BH201 */
	...
	if (gpio_is_valid(pdata->pwr_gpio)) {
		dev_err(dev, " gpio: %d is for Redriver power enable gpio\n", pdata->pwr_gpio);
	} else {
		dev_err(dev, "Current project is Not support Redriver .....\n");
	}
/* #endif CONFIG_MMC_SDHCI_BH201 */
...
static irqreturn_t sdhci_msm_pwr_irq(int irq, void *data)
{
	...
	if ((mmc->power_before_card || mmc->card) && mmc->ops->get_cd
		&& !mmc->ops->get_cd(mmc) && irq_status & CORE_PWRCTL_BUS_ON) {
		pr_err("%s, %s: card is not present and CORE_PWRCTL_BUS_ON\n", __func__, mmc_hostname(msm_host->mmc));
...
/* #ifdef CONFIG_MMC_SDHCI_BH201 */
		sdhci_bht_power(host, msm_host->pdata->status_gpio, msm_host->pdata->pwr_gpio, 1);
/* #endif CONFIG_MMC_SDHCI_BH201 */
	}
	if (irq_status & CORE_PWRCTL_BUS_OFF) {
		if (msm_host->pltfm_init_done) {
/* #ifdef CONFIG_MMC_SDHCI_BH201 */
			sdhci_bht_power(host, msm_host->pdata->status_gpio, msm_host->pdata->pwr_gpio, 0);
/* #endif CONFIG_MMC_SDHCI_BH201 */
...
static void sdhci_msm_check_power_status(struct sdhci_host *host, u32 req_type)
{
	...
	if ((mmc->power_before_card || mmc->card) && mmc->ops->get_cd
		...
		pr_err("%s, %s: card is not present and REQ_BUS_ON\n", __func__, mmc_hostname(msm_host->mmc));
		host->pwr = 0;
...
/* #endif CONFIG_MMC_SDHCI_BH201 */
	 ...
	 */
		...
		writel_relaxed(readl_relaxed(host->ioaddr +
				...
				& ~CORE_CLK_PWRSAVE, host->ioaddr +
				msm_host_offset->CORE_VENDOR_SPEC);
	} /* CONFIG_MMC_SDHCI_BH201 add this line */
...
/* #ifdef CONFIG_MMC_SDHCI_BH201 */
	if (gpio_is_valid(msm_host->pdata->pwr_gpio)) {
		sdhci_msm_ops.set_timeout = sdhci_bht_set_timeout;
		sdhci_msm_ops.platform_execute_tuning = sdhci_bht_execute_tuning;
		host->mmc->redriver_en = true;
		ret = devm_gpio_request_one(msm_host->mmc->parent, msm_host->pdata->pwr_gpio, GPIOF_OUT_INIT_LOW, "redrive_pwr_en");
		if (ret < 0) {
			dev_err(&pdev->dev, "%s: Failed to request pwr gpio %d\n", __func__, ret);
		}
	} else {
		host->mmc->redriver_en = false;
	}
/* #endif CONFIG_MMC_SDHCI_BH201 */
