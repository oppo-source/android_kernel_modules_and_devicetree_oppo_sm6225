...
void sdhci_bht_power(struct sdhci_host *host, int status_gpio, int pwr_gpio, int on)
{
	...
	if (!gpio_is_valid(status_gpio)) {
		pr_err("%s: no det_gpio provided, just return\n", mmc_hostname(host->mmc));
		return;
	}

	card_present_status = gpio_get_value(status_gpio);
	pr_info("%s: det_gpio pin %d status is %d\n", mmc_hostname(host->mmc),
		status_gpio, card_present_status);
	...
	if (bht_target_host(host)) {
		...
		ggc_chip_init(host);
		gpio_set_value(pwr_gpio, on);  /* set PWR GPIO */
