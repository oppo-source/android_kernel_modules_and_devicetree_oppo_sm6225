	...
	if (bht_target_host(host)) {
		...
		if (vendor_host->ggc.driver_strength_reinit_flg) {
			...
			if (vendor_host->ggc.driver_strength_reinit_flg <= 7) {
				...
				gg_sw_def[15] |= (driver_strength_reinit_flg << 28)	| (driver_strength_reinit_flg << 20);
			}
		}
...
/* #ifdef CONFIG_MMC_SDHCI_BH201 */
	if (host->redriver_en) {
retry:
		if (bht_target_host(mmc_priv(host))) {
			int retries = 5;
			...
			while (retries) {
				...
				/* card failed at identify stage, bad card */
				if (host->cur_sd_bus_speed < 2) {
					...
					/* card failed at tuning stage, need retry */
					retries--;
			...
			/* card failed at tuning stage and set retry, need degrade */
			if (err) {
				host->degrade = 1;
