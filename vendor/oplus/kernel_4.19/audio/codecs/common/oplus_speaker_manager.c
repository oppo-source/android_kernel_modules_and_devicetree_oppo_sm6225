...
static const struct snd_kcontrol_new oplus_pa_manager_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Amp_Force_Mute", (oplus_amp_control_enum)[5], ext_amp_force_mute_get, ext_amp_force_mute_set),
};
...
{
	enum oplus_pa_manufacturer mrf = MFR_UNKONW;
...
{
	enum oplus_pa_manufacturer value = 0;

	value = oplus_speaker_mfr_get(L_SPK);

	ucontrol->value.integer.value[0] = (value >= MFR_UNKONW) ? value : 0;

	pr_warning("%s, %d, status = %d\n", __func__, __LINE__, ucontrol->value.integer.value[0]);
...
int speaker_r_mfr_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	value = oplus_speaker_mfr_get(R_SPK);
...
int ext_amp_force_mute_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (contrl_status == NULL) {
		...
		pr_debug("%s(), contrl_status->amp_force_mute_status = %d\n", __func__, contrl_status->amp_force_mute_status);
	}
